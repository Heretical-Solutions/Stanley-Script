...
namespace UImGui.Renderer
{
	internal sealed class RendererMesh : IRenderer
	{
		...
		{
			Vector2 fbOSize = (drawData.DisplaySize * drawData.FramebufferScale).ToUnityVector2();
		...
		private void CreateDrawCommands(CommandBuffer commandBuffer, ImDrawDataPtr drawData, Vector2 fbSize)
		{
			...
			System.Numerics.Vector4 clipOffset = new System.Numerics.Vector4(
				drawData.DisplayPos.X,
				drawData.DisplayPos.Y,
				drawData.DisplayPos.X, 
				drawData.DisplayPos.Y);
			
			System.Numerics.Vector4 clipScale = new System.Numerics.Vector4(
			...
			for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
			{
				...
				for (int i = 0, iMax = drawList.CmdBuffer.Size; i < iMax; ++i, ++subOf)
				{
					...
					else
					{
						...
						System.Numerics.Vector4 clipSize = drawCmd.ClipRect - clipOffset;
						System.Numerics.Vector4 clip = clipSize * clipScale; //Vector4.Scale(clipSize, clipScale);
